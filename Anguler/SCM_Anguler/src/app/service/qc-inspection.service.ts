...
import { QCInspectionRequestModel, QCInspectionResponseModel } from '../component/shared/model/qc-inspection';
...
export class QcInspectionService {
  ...
  save(inspection: QCInspectionRequestModel, file: File | null): Observable<QCInspectionResponseModel> {
  ...
  update(id: number, inspection: QCInspectionRequestModel, file: File | null): Observable<QCInspectionResponseModel> {
    const formData = new FormData();
    formData.append('inspection', JSON.stringify(inspection));
    if (file) {
      formData.append('file', file);
